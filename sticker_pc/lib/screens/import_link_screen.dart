...
class _ImportLinkScreenState extends State<ImportLinkScreen> {
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: IconButton(
                  ...
                  onPressed: () async {
                    ...
                    if (data?.text != null) _codeController.text = data!.text!;
                  },
                ),
              ),
              maxLines: 3,
                  ...
                  child: Column(children: [
                    ...
                    if (_parsedLink != null) ...[
                      ...
                      Container(
                        ...
                        child: const Text('端到端加密', style: TextStyle(fontSize: 12, color: Colors.green)),
                      ),
                    ],
                  ]),
                ),
    ...
    try {
      ...
      if (_parsedLink != null) {
        ...
        final result = await dl.downloadPack(
          ...
          onProgress: (done, total) { if (mounted) setState(() { _downloadProgress = done / total; }); },
        );
        if (!mounted) return;
  ...
  Future<void> _downloadEncrypted(StorageService storage, PackProvider packProvider, ScaffoldMessengerState messenger) async {
    ...
    if (remoteStickers.isEmpty) { setState(() { _error = '远程包为空'; }); return; }
    pack.source = 'link';
    ...
    if (!mounted) return;
    if (stickers.isEmpty) { setState(() { _error = '下载/解密失败，请检查链接是否正确'; }); return; }
    final msg = failedCount > 0 ? '下载完成，${stickers.length} 个成功，$failedCount 个失败' : '下载完成，共 ${stickers.length} 个表情';
    packProvider.loadPacks();
    messenger.showSnackBar(SnackBar(content: Text(msg)));
    Navigator.pop(context);
  }
