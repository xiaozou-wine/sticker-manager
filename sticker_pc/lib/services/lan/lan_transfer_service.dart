...
class LanTransferService {
  ...
  Future<void> _handleSendRequest(HttpRequest request) async {
    debugPrint('[LAN] Send request from ${request.connectionInfo?.remoteAddress}');
    if (_isBusy) {
      debugPrint('[LAN] Send rejected: busy');
    ...
    try {
      ...
      debugPrint('[LAN] Send request ${accepted ? "ACCEPTED" : "DECLINED"}');
      if (accepted) {
